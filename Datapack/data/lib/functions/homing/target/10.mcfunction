#> lib:homing/target/10
# ホーミングする敵の特定(Rotation誤差±10度)
...
# Rotationの誤差±10度以上の敵はタグ剥奪
    execute positioned ^ ^ ^10 as @e[type=#lib:every_mob,tag=Target,distance=..10] positioned ^ ^ ^-10 rotated ~10 ~ positioned ^1000 ^ ^ unless entity @s[distance=..1000] run tag @s remove Target
    execute positioned ^ ^ ^10 as @e[type=#lib:every_mob,tag=Target,distance=..10] positioned ^ ^ ^-10 rotated ~-10 ~ positioned ^-1000 ^ ^ unless entity @s[distance=..1000] run tag @s remove Target
    execute positioned ^ ^ ^10 as @e[type=#lib:every_mob,tag=Target,distance=..10] positioned ^ ^ ^-10 rotated ~ ~10 positioned ^ ^1000 ^ unless entity @s[distance=..1000] run tag @s remove Target
    execute positioned ^ ^ ^10 as @e[type=#lib:every_mob,tag=Target,distance=..10] positioned ^ ^ ^-10 rotated ~ ~-10 positioned ^ ^-1000 ^ unless entity @s[distance=..1000] run tag @s remove Target
...
# ホーミング対象が居なければ範囲を広めてもう一度
    execute unless entity @e[tag=HomingTarget,distance=..20,limit=1] run function lib:homing/target/20