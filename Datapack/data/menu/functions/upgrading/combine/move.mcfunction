...
# 合成アイテム(+板ガラス)を鍛冶台のタグに入れる
    data modify storage menu:temp Data.smithing set value {Count:1b,Slot:14b,id:"smithing_table",tag:{menu:{blank:1b,button:"combine"},display:{Name:'{"text":"合成","bold":true,"italic":false}'}}}
...
# 演出
    ...
    execute unless score $CombineIngredients Temporary matches 0 run playsound block.grindstone.use master @p ~ ~ ~ 1 1.5