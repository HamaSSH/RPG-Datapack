...
# 1. 目前のVex
    ...
    execute unless entity @s[tag=DisableLC] unless entity @s[tag=Block] run summon vex ~ ~1 ~-0.3 {PortalCooldown:1,Silent:1b,Team:"NoCollision",NoAI:1b,Health:1000f,Tags:["LeftClick"],Attributes:[{Name:"generic.max_health",Base:1000}]}
# 2. ^^^0.5のVex
    execute unless entity @s[tag=DisableLC] if entity @s[tag=Block] run summon minecraft:vex ~ ~1 ~ {PortalCooldown:1,Silent:1b,Team:"NoCollision",NoAI:1b,Health:1000f,Tags:["LeftClick"],Attributes:[{Name:"generic.max_health",Base:1000}]}
    tag @s remove Block
