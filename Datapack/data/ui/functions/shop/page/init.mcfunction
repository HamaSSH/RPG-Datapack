...
# 商品ページを移る用アイテムを配置するか判定
    ...
    execute on passengers if score @s ShopPage matches 1.. on vehicle run data modify entity @s Items append value {Slot:9b,id:"glass_bottle",components:{"minecraft:custom_model_data":3014,"minecraft:custom_data":{ui:{item_type:"backward"}},display:{Name:'{"text":"←","color":"white","bold":true,"italic":false}'}}}
    execute on passengers if score @s ShopPage < $TotalPage Temporary on vehicle run data modify entity @s Items append value {Slot:16b,id:"glass_bottle",components:{"minecraft:custom_model_data":3015,"minecraft:custom_data":{ui:{item_type:"forward"}},display:{Name:'{"text":"→","color":"white","bold":true,"italic":false}'}}}
