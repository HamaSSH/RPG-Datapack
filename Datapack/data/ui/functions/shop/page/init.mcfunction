...
# 商品ページを移る用アイテムを配置するか判定
    ...
    execute on passengers if score @s ShopPage matches 1.. on vehicle run data modify entity @s Items append value {Count:1b,Slot:9b,id:"glass_bottle",tag:{CustomModelData:3014,UI:{ItemType:"Backward"},display:{Name:'{"text":"←","color":"white","bold":true,"italic":false}'}}}
    execute on passengers if score @s ShopPage < $TotalPage Temporary on vehicle run data modify entity @s Items append value {Count:1b,Slot:16b,id:"glass_bottle",tag:{CustomModelData:3015,UI:{ItemType:"Forward"},display:{Name:'{"text":"→","color":"white","bold":true,"italic":false}'}}}
