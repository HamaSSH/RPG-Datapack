...
# エフェクトがダブっているか/最大値を超えているか検知
    ...
    $execute store result storage ui: Ingredient.Buff[0].ID int 1 run data get storage ui: PotionData.Buff[{Effect:"$(Effect)"}].ID
...
# 新しいエフェクトはそのままエフェクトを付与
    ...
    execute unless data storage ui: PotionData{Duplicate:1b} if score $EffectCount Temporary matches ..1 run data modify storage ui: PotionData.EffectLore append from storage ui: Ingredient.display.Lore[3]
