...
# エフェクトがダブっているか検知
    execute store result score $StatusCitation Temporary if data storage ui: Ingredient.Buff[].Status
...
# 新しいステータスバフはそのままステータスバフを付与
    ...
    execute unless data storage ui: PotionData{Duplicate:1b} run function ui:brewing/brew/potion/buff/status/add with storage ui: Ingredient.Buff[0]
...
# 処理が終わっていないバフが無くなるまで再帰
    scoreboard players reset $StatusCitation Temporary
