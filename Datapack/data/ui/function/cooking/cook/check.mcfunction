...
# 素材アイテムのIDをstorageに格納
    ...
    data modify storage ui: CookIngredients.ingredient1 set from storage ui: NewItems[{Slot:10b}].components.minecraft:custom_data.item_id
    data modify storage ui: CookIngredients.ingredient2 set from storage ui: NewItems[{Slot:11b}].components.minecraft:custom_data.item_id
    data modify storage ui: CookIngredients.ingredient3 set from storage ui: NewItems[{Slot:12b}].components.minecraft:custom_data.item_id

# 結果取得
    function ui:cooking/recipe/get with storage ui: CookIngredients
    execute unless data storage ui: Result.id run tag @s remove Cookable
