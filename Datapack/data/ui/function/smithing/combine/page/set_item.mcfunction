...
$execute if data storage ui: Items[{Slot:$(Slot),tag:{UI:{Combinable:1b}}}] run data modify storage ui: CombineItem set from storage ui: Items[{Slot:$(Slot)}]

# 空きがあればスロットに配置(予定)
$execute unless data storage ui: Items[{Slot:$(Slot),tag:{UI:{Combinable:1b}}}] run data modify storage ui: CombinableItems[0].Slot set value $(Slot)
