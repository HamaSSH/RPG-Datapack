...
# 会心攻撃力上昇率 DmgDealt = DmgDealt * (CRT/10 + 130) / 100
    # = DmgDealt * (CRT + 1300) / 1000
        ...
        scoreboard players operation $DmgDealt Temporary *= $CritMultiplier Temporary
    # 一の位四捨五入
        scoreboard players add $DmgDealt Temporary 500
        scoreboard players operation $DmgDealt Temporary /= #1000 Constant
