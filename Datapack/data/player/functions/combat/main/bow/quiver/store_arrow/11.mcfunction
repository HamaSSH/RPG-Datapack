...
# スロット11の矢筒のデータ
    execute store result score $Capacity Temporary run data get storage lib: NewInventory[{Slot:11b}].components.minecraft:custom_data.quiver.capacity
    execute store result score $Arrow Temporary run data get storage lib: NewInventory[{Slot:11b}].components.minecraft:custom_data.quiver.arrow
...
# 矢筒のArrowタグに設定する弓矢の本数
    ...
    execute store result storage lib: NewInventory[{Slot:11b}].components.minecraft:custom_data.quiver.arrow int 1 run scoreboard players get $Arrow Temporary

# Loreの設定
    function player:combat/main/bow/quiver/store_arrow/lore/11 with storage lib: NewInventory[{Slot:11b}].components.minecraft:custom_data.quiver
