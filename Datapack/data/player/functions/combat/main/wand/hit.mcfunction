...
# ダメージ処理
    # 被ダメージ(補正無し)を設定
    scoreboard players operation @s DmgReceived = @e[type=area_effect_cloud,tag=WandAEC,tag=Attacker,sort=nearest,limit=1] DmgDealt
...
# 攻撃したプレイヤーを記録
    scoreboard players operation @s PlayerID = @e[type=area_effect_cloud,tag=WandAEC,tag=Attacker,sort=nearest,limit=1] PlayerID
