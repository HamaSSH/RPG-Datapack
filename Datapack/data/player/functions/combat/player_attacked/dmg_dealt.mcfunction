...
# 物理攻撃・魔法攻撃・器用攻撃に応じたステータス
    ...
    execute if entity @s[advancements={player:combat/player_attacked={projectile=true}}] run scoreboard players operation @s DmgDealt = @s DEX
...
# チャージ率に応じてダメージ補正(projectile)
    ...
    execute if entity @s[advancements={player:combat/player_attacked={projectile_charge2=true}}] run scoreboard players operation @s DmgDealt /= #10 Constant
    execute if entity @s[advancements={player:combat/player_attacked={projectile_charge3=true}}] run scoreboard players operation @s DmgDealt /= #8 Constant
