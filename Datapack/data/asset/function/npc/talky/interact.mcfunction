...
# プレイヤーごとの会話文管理
    ...
    execute as @n[type=villager,tag=talky] run function asset:npc/dialogue/get_id with storage player:

# 会話文の表示
    execute store result score $DialogueID Temporary run data get storage npc: dialogue.id
    function asset:npc/talky/dialogue/_ with storage npc: dialogue
    execute store result storage npc: dialogue.id int 1 run scoreboard players get $DialogueID Temporary

# プレイヤーごとの会話文の進捗を保存
    execute as @n[type=villager,tag=talky] run function asset:npc/dialogue/set_id with storage player:
...
# リセット
    ...
    advancement revoke @s only asset:npc/talky