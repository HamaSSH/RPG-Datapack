#> asset:magic/5000.example/hit/_
# 魔法がヒットした時の処理

# 周りの敵にも当たる攻撃
    scoreboard players set $XPower Temporary 80
    scoreboard players set $YPower Temporary 80
    execute at @s positioned ~-0.8 ~-0.8 ~-0.8 as @e[type=#lib:every_mob,tag=Enemy,tag=!HurtTime,dx=0.6,dy=0.6,dz=0.6,sort=nearest,limit=5] run function asset:magic/5000.example/hit/aoe
# 演出
