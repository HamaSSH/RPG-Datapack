...
# 分裂由来ではない召喚
    # サイズランダム
        execute unless score $SlimeSize Temporary matches 0.. run scoreboard players set $RNGMax Temporary 3
        execute unless score $SlimeSize Temporary matches 0.. run function core:rng
        execute unless score $SlimeSize Temporary matches 0.. run scoreboard players operation $SlimeSize Temporary = $RNG Temporary
...
# リセット
    execute if score $RNG Temporary matches 0.. run scoreboard players reset $SlimeSize Temporary
