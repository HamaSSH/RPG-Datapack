...
# バフの種類ごとの置き換え
    execute if data storage mob:temp Data.buff{bonus:"ad"} if data storage mob:temp Data.oldBuff{bonus:"ad"} run scoreboard players operation @e[tag=Enemy,tag=this,limit=1] BuffAD -= $Bonus Temporary
    ...
    execute if data storage mob:temp Data.buff{bonus:"def"} if data storage mob:temp Data.oldBuff{bonus:"def"} run scoreboard players operation @e[tag=Enemy,tag=this,limit=1] BuffDEF -= $Bonus Temporary
    ...
    execute if data storage mob:temp Data.buff{bonus:"spd"} if data storage mob:temp Data.oldBuff{bonus:"spd"} run scoreboard players operation @e[tag=Enemy,tag=this,limit=1] BuffSPD -= $Bonus Temporary
