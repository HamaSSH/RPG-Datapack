...
# モブ固有のtick処理
    execute if data entity @s {data:{triggers:["tick"]}} run data modify storage macro:temp mob.id set from entity @s data.id
    execute if data entity @s {data:{triggers:["tick"]}} run function mob:macro/tick.m with storage macro:temp mob
