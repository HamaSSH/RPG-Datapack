# 状態異常のパーティクル
    execute if entity @s[tag=Weakness] at @e[tag=this,limit=1] run particle minecraft:bubble_pop ~ ~0.5 ~ 0.2 0.3 0.2 0 10 force
    execute if entity @s[tag=Paralize] at @e[tag=this,limit=1] run particle minecraft:dust 1000000000 1000000000 100 2 ~ ~0.9 ~ 0.2 0.1 0.2 0 7 force
...
# durationが0の場合はkill
    execute if data storage mob:temp Data.buff{duration:0} as @e[tag=this,limit=1] run function mob:buff/remove
