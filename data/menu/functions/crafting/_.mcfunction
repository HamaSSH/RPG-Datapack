...
# メニュー更新
    ...
    # execute if score @s page matches 1 positioned ~ ~-2 ~ as @e[type=chest_minecart,tag=Open,distance=..5] positioned ~ ~2 ~ if score @s PlayerID = @p PlayerID run function menu:crafting/evolve/refresh
    # execute if score @s page matches 2 positioned ~ ~-2 ~ as @e[type=chest_minecart,tag=Open,distance=..5] positioned ~ ~2 ~ if score @s PlayerID = @p PlayerID run function menu:crafting/combine/refresh
# メニュー内の操作音
