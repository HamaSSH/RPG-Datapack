...
# ボタンクリック
    ...
    execute unless data storage menu:temp Data.Items[{Slot:9b,tag:{menu:{blank:1b}}}] positioned ~ ~-2 ~ as @e[type=chest_minecart,tag=Open,distance=..5] positioned ~ ~2 ~ if score @s PlayerID = @p PlayerID run function menu:cooking/refresh
    execute unless data storage menu:temp Data.Items[{Slot:13b,tag:{menu:{blank:1b}}}] positioned ~ ~-2 ~ as @e[type=chest_minecart,tag=Open,distance=..5] positioned ~ ~2 ~ if score @s PlayerID = @p PlayerID run function menu:cooking/refresh
    execute unless data storage menu:temp Data.Items[{Slot:14b,tag:{menu:{button:"cook"}}}] run function menu:cooking/cook/check
    execute unless data storage menu:temp Data.Items[{Slot:17b,tag:{menu:{blank:1b}}}] positioned ~ ~-2 ~ as @e[type=chest_minecart,tag=Open,distance=..5] positioned ~ ~2 ~ if score @s PlayerID = @p PlayerID run function menu:cooking/refresh
# メニュー内の操作音
