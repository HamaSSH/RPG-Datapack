# 弓を撃った人のタグ付け
    tag @s add this
# 撃った弓矢としての処理
    execute anchored eyes positioned ^ ^ ^ positioned ~ ~-0.1 ~ as @e[type=arrow,distance=..0.6] run function combat:main/bow/init
    execute if score @s skill_cd matches 0 if score @s BowCharge matches 40.. run function combat:main/bow/skill/cost
# 矢筒を持っていたらスコア減算
