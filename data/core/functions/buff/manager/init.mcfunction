...
# 状態異常のデータがあれば格納
    execute if data storage core:temp Data.buff{source:"paralize"} run tag @s add Paralize
    execute if data storage core:temp Data.buff{source:"weakness"} run tag @s add Weakness
