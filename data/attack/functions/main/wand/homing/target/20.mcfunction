...
# Rotationの誤差±20度以上の敵はタグ剥奪
    execute positioned ^ ^ ^10 as @e[type=#mob:mobs,tag=Enemy,tag=!HurtTime,distance=..10] positioned ^ ^ ^-10 rotated ~20 ~ positioned ^1000 ^ ^ unless entity @s[distance=..1000] run tag @s remove Target
    execute positioned ^ ^ ^10 as @e[type=#mob:mobs,tag=Enemy,tag=!HurtTime,distance=..10] positioned ^ ^ ^-10 rotated ~-20 ~ positioned ^-1000 ^ ^ unless entity @s[distance=..1000] run tag @s remove Target
    execute positioned ^ ^ ^10 as @e[type=#mob:mobs,tag=Enemy,tag=!HurtTime,distance=..10] positioned ^ ^ ^-10 rotated ~ ~20 positioned ^ ^1000 ^ unless entity @s[distance=..1000] run tag @s remove Target
    execute positioned ^ ^ ^10 as @e[type=#mob:mobs,tag=Enemy,tag=!HurtTime,distance=..10] positioned ^ ^ ^-10 rotated ~ ~-20 positioned ^ ^-1000 ^ unless entity @s[distance=..1000] run tag @s remove Target
# ホーミング対象を絞る
...
# ホーミング対象が居なければ範囲を広めてもう一度
    execute unless entity @e[tag=HomingTarget,distance=..20,limit=1] run function attack:main/wand/homing/target/30