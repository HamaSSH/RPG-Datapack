...
# MPコスト
  ...
  execute if entity @s[tag=MPCheck] if score $Angle Temporary matches -60..90 run function attack:main/bow/skill/1/init
  execute if entity @s[tag=MPCheck] unless score $Angle Temporary matches -60..90 run function attack:main/bow/skill/2/init
# リセット
