...
# ノックバック
  scoreboard players set $XPower Temporary 90
  scoreboard players set $YPower Temporary 90
# 当たり判定
  execute anchored eyes positioned ^ ^ ^-0.1 run function attack:main/dagger/rec
  execute as @e[type=#mob:mobs,tag=Hit,tag=Enemy,tag=!HurtTime,distance=..3.5,sort=nearest,limit=2] run function attack:main/dagger/hit
