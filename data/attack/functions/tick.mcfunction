...
# 攻撃の#tick
    execute as @a at @s if score @s skill_cd matches 1.. run function attack:main/tick
...
# 時間経過でkillするエンティティ
    execute as @e[type=panda,tag=LeftClick] if data entity @s {PortalCooldown:0} run function attack:main/kill
