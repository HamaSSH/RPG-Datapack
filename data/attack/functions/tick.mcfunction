...
# 弓矢関連
  ...
  execute as @e[type=arrow,tag=!ArrowLand] if data entity @s {inGround:1b} run function attack:left_click/bow/arrow/land
...
# 時間経過でkillするエンティティ
  ...
  execute as @e[type=arrow,tag=ArrowLand] at @s if data entity @s {inGround:1b,PortalCooldown:0} run function attack:left_click/bow/arrow/kill
