# モブに攻撃したプレイヤーをタグ付け
    tag @s add this
# モブの絞り込み
    execute anchored eyes positioned ^ ^ ^ positioned ~ ~-0.8 ~ if entity @e[type=panda,tag=LeftClick,distance=..0,sort=nearest,limit=1] at @s if entity @s[advancements={combat:player_attacked={projectile=false}}] run function combat:main/_
